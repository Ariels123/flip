...
### Port Convention (CONSISTENT ACROSS ALL SYSTEMS)
| Environment | Port | Description |
|-------------|------|-------------|
| Production | 8090 | ALL systems use 8090 for production |
| Test | 9190 | ALL systems use 9190 for test |

**IMPORTANT**: Same port on every machine. No exceptions.
- Mac Production: http://192.168.1.53:8090
- Windows Production: http://192.168.1.220:8090
- Mac Test: http://192.168.1.53:9190
- Windows Test: http://192.168.1.220:9190
...
## Rule 6: Test vs Production Safeguards

**CRITICAL: Never accidentally modify production when intending to modify test.**

### Visual Indicators
1. **Log Files**: Test server logs to `/tmp/flip2d-test.log`, production to `/tmp/flip2d.log`
2. **PID Files**: Test uses `/tmp/flip2d-test.pid`, production uses `/tmp/flip2d.pid`
3. **Data Directories**: Test uses `./pb_data_test/`, production uses `./pb_data/`
...
- `FLIP2_ENV=production` (default) or `FLIP2_ENV=test`
- Code should check this and log prominently on startup

### Verification Before ANY Work
Before making changes, ALWAYS verify which environment you're targeting:
```bash
# Check which port is responding
curl -s http://localhost:8090/api/health  # Production
curl -s http://localhost:9190/api/health  # Test

# Check running processes
ps aux | grep flip2d
lsof -i :8090  # Production port
lsof -i :9190  # Test port
```

### Safeguard Checklist
Before ANY structural change:
- [ ] Verified which port I'm connecting to
- [ ] Confirmed config file being used
- [ ] Checked ps aux for running daemons
- [ ] Confirmed environment (test vs prod)
- [ ] Made changes on TEST first
- [ ] Tested changes work on TEST
- [ ] ONLY THEN applied to production

### Incident: 2025-12-20
- Accidentally worked on production (8090/8091) instead of test (9190)
- Root cause: Not verifying environment before starting work
- Prevention: Always run verification commands before any work

---

## Version History
- v1.1 (2025-12-20): Added Rule 6 (Test vs Production Safeguards) after incident
